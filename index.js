...
function Linux(instance, end) {
  LinuxBinary(instance, (error, binary) => {
...
function Mac(instance, callback) {
  ...
  UUID(instance, (error, uuid) => {
    ...
    function end(error, stdout, stderr) {
      Remove(Node.path.dirname(instance.path), (errorRemove) => {
        if (error) return callback(error);
    ...
    MacApplet(instance, (error, stdout, stderr) => {
      ...
      MacIcon(instance, (error) => {
        if (error) return end(error);
        MacPropertyList(instance, (error, stdout, stderr) => {
          if (error) return end(error, stdout, stderr);
          MacCommand(instance, (error) => {
            if (error) return end(error);
            MacOpen(instance, (error, stdout, stderr) => {
              if (error) return end(error, stdout, stderr);
...
function MacApplet(instance, end) {
  ...
  Node.fs.mkdir(parent, (error) => {
    if (error) return end(error);
    var zip = Node.path.join(parent, "sudo-prompt-applet.zip");
    Node.fs.writeFile(zip, APPLET, "base64", (error) => {
      if (error) return end(error);
      var command = [];
...
function MacIcon(instance, end) {
  ...
  Node.fs.readFile(instance.options.icns, (error, buffer) => {
    if (error) return end(error);
...
function MacResult(instance, end) {
  ...
  Node.fs.readFile(Node.path.join(cwd, "code"), "utf-8", (error, code) => {
    if (error) {
...
function UUID(instance, end) {
  Node.crypto.randomBytes(256, (error, random) => {
...
function Windows(instance, callback) {
  ...
  UUID(instance, (error, uuid) => {
    ...
    Node.fs.mkdir(instance.path, (error) => {
      if (error) return callback(error);
      function end(error, stdout, stderr) {
        Remove(instance.path, (errorRemove) => {
          if (error) return callback(error);
          if (errorRemove) return callback(errorRemove);
          callback(undefined, stdout, stderr);
        });
      }
      WindowsWriteExecuteScript(instance, (error) => {
        if (error) return end(error);
        WindowsWriteCommandScript(instance, (error) => {
          if (error) return end(error);
          WindowsElevate(instance, (error, stdout, stderr) => {
            if (error) return end(error, stdout, stderr);
            WindowsWaitForStatus(instance, (error) => {
              if (error) return end(error);
...
function WindowsResult(instance, end) {
  Node.fs.readFile(instance.pathStatus, "utf-8", (error, code) => {
    if (error) return end(error);
    Node.fs.readFile(instance.pathStdout, "utf-8", (error, stdout) => {
      if (error) return end(error);
      Node.fs.readFile(instance.pathStderr, "utf-8", (error, stderr) => {
        if (error) return end(error);
...
function WindowsWaitForStatus(instance, end) {
  ...
  Node.fs.stat(instance.pathStatus, (error, stats) => {
    if ((error && error.code === "ENOENT") || stats.size < 2) {
      ...
      setTimeout(() => {
        ...
        Node.fs.stat(instance.pathStdout, (error) => {
